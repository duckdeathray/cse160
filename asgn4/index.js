...
function tick(){
    ...
    if(animateLight){
        lightAngle += .01;
        lightPos[0] = 1.5 * Math.cos(lightAngle) * 2.0;
        lightPos[2] = 1.5 * Math.sin(lightAngle) * 2.0;
        lightPos[1] = 1.5 + Math.sin(lightAngle * 0.5) * 0.5;
    }
...
function renderScene(){
    ...
    gl.uniform1i(u_LightingEnabled, lightingEnabled ? 1 : 0);
    ...
    sphere.matrix.translate(1.5, 0, 3);
    sphere.matrix.scale(.3, .3, .3);
    sphere.render();

    let lightCube = new Cube();
    lightCube.useTexture = false;
...
function main() {
    ...
    document.getElementById("light").oninput = function(ev){
        lightPos[0] = parseFloat(ev.target.value);
    };

    document.getElementById("lightR").oninput = function(ev){
        lightColor[0] = parseFloat(ev.target.value);
    };

    document.getElementById("lightG").oninput = function(ev){
        lightColor[1] = parseFloat(ev.target.value);
    };

    document.getElementById("lightB").oninput = function(ev){
        lightColor[2] = parseFloat(ev.target.value);
    };
